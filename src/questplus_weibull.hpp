...
private:
    Weibull::Params sanitize(em::val params)
    {
        ...
        em::val threshold = params["threshold"];
        if (threshold.isArray())
        {
            p.threshold = em::convertJSArrayToNumberVector<double>(threshold);
        }
        em::val slope = params["slope"];
        if (slope.isArray())
        {
            p.slope = em::convertJSArrayToNumberVector<double>(slope);
        }
        ...
        // priors
        em::val threshold_prior = params["threshold_prior"];
        if (threshold_prior.isArray())
        {
            p.threshold_prior = em::convertJSArrayToNumberVector<double>(threshold_prior);
        }
        em::val slope_prior = params["slope_prior"];
        if (slope_prior.isArray())
        {
            p.slope_prior = em::convertJSArrayToNumberVector<double>(slope_prior);
        }
...
public:
    ...
    {
        return Weibull::update(value);
    }
    bool update(int value, double stimulus)
    {
        return Weibull::update(value, stimulus);
    }
